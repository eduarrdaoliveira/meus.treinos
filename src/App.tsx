...
import { CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Dumbbell, Layers3, LogOut, Pencil, Plus, Save, Trash2, TrendingUp, UserRound } from 'lucide-react';
...
function SetTable({sets,onChange}:{sets:SetRow[];onChange:(j:number,key:keyof SetRow,value:string|boolean)=>void}){
 return <>
  ...
  {sets.map((s,j)=><div className="setRow" style={j%2===1?{background:'var(--surface-2)'}:undefined} key={j}>
    <span>{j+1}</span>
...
export default function App(){
 ...
 const [hojeStarted,setHojeStarted]=useState(false);
 ...
 useEffect(()=>{if(!user)return;if(activeItems.length||selectedTemplate)return;const sched=schedules.find(s=>isScheduledOn(s,new Date(today()+'T12:00:00')));if(sched){loadTemplate(sched.templateId);setHojeStarted(true);}},[schedules,user]);
 ...
 function startScheduledWorkout(dateStr:string,templateId:string){loadTemplate(templateId);setWorkoutDate(dateStr);setHojeStarted(true);setTab('today');}
 function startFreeWorkoutOn(dateStr:string){setSelectedTemplate('');setActiveItems([]);setWorkoutDate(dateStr);setHojeStarted(true);setTab('today');}
 ...
 return <div className="app">
  ...
  {tab==='today'&&<main>
   {!hojeStarted&&!activeItems.length&&!selectedTemplate?<section className="panel emptyToday">
     <div className="empty">Nenhum treino programado para hoje!</div>
     <button onClick={()=>setHojeStarted(true)}><Plus size={18}/> Iniciar treino</button>
    </section>:<>
  ...
  {tab==='exercises'&&<main className="grid2"><section className="panel"><h2>{exerciseForm.id?'Editar exercício':'Novo exercício'}</h2><label>Nome<input value={exerciseForm.name} onChange={e=>setExerciseForm({...exerciseForm,name:e.target.value})}/></label><label>Grupo<select value={exerciseForm.group} onChange={e=>setExerciseForm({...exerciseForm,group:e.target.value})}>{groups.map(g=><option key={g}>{g}</option>)}</select></label><label>Equipamento<input value={exerciseForm.equipment} onChange={e=>setExerciseForm({...exerciseForm,equipment:e.target.value})}/></label><label>Observações<textarea value={exerciseForm.notes} onChange={e=>setExerciseForm({...exerciseForm,notes:e.target.value})}/></label><button onClick={saveExercise}><Save size={18}/> Salvar</button></section><section><h2>Biblioteca</h2><div className="compactList">{exercises.map(e=><div className="compactRow" key={e.id}><div className="compactInfo"><span className="compactName">{e.name}</span><span className="compactGroup">{e.group}</span></div><div className="compactActions"><button className="secondary" onClick={()=>setExerciseForm(e)}>Editar</button><button className="danger" onClick={()=>confirm('Excluir exercício?')&&deleteDoc(doc(db,base,'exercises',e.id))}>Excluir</button></div></div>)}</div></section></main>}

  {tab==='templates'&&<main className="grid2"><section className="panel"><h2>{templateForm.id?'Editar modelo':'Novo modelo'}</h2><label>Nome<input value={templateForm.name} onChange={e=>setTemplateForm({...templateForm,name:e.target.value})}/></label><div className="checks">{exercises.map(e=><label key={e.id}><input type="checkbox" checked={templateForm.exerciseIds.includes(e.id)} onChange={()=>setTemplateForm(f=>({...f,exerciseIds:f.exerciseIds.includes(e.id)?f.exerciseIds.filter(x=>x!==e.id):[...f.exerciseIds,e.id]}))}/>{e.name}</label>)}</div><button onClick={saveTemplate}><Save size={18}/> Salvar modelo</button></section><section><h2>Modelos</h2><div className="compactList">{templates.map(t=><div className="compactRow" key={t.id}><div className="compactInfo"><b>{t.name}</b><span className="compactSub">{t.exerciseIds.length} exercícios</span></div><div className="compactActions"><button className="secondary" onClick={()=>{setTemplateForm(t);scrollTo(0,0)}}>Editar</button><button className="danger" onClick={()=>confirm('Excluir modelo?')&&deleteDoc(doc(db,base,'templates',t.id))}>Excluir</button></div></div>)}</div></section></main>}
  ...
  {tab==='evolution'&&<main>
   ...
   <section className="panel">
    <div className="panelHead"><h3>Evolução por exercício específico</h3><select value={evoExercise} onChange={e=>setEvoExercise(e.target.value)}><option value="">Escolher exercício</option>{exercises.map(e=><option value={e.id} key={e.id}>{e.name}</option>)}</select></div>
    {!evoExercise?<div className="empty">Escolha um exercício para ver mais detalhes.</div>:<ExerciseEvolution workouts={workouts} exerciseId={evoExercise}/>}
   </section>
  </main>}
...
function LineChart({data}:{data:{date:string;value:number}[]}){
 ...
 const pts=data.map((d,i)=>({x:padL+(data.length>1?i*stepX:plotW/2),y:padT+plotH-(d.value/max)*plotH,...d}));
 ...
 return <div className="evoChartWrap">
  <svg width={w} height={chartH} viewBox={`0 0 ${w} ${chartH}`}>
   ...
   {pts.length>1&&<polyline points={linePoints} fill="none" stroke="#008B8B" strokeWidth={2.5} strokeLinejoin="round" strokeLinecap="round"/>}
